...
|---|---|
| `app.bsky.feed.post` | Text, embeds, reply context, Bluesky link |
...
Adding a new formatter is a few lines — see the `FORMATTERS` object in `src/index.js`.

---

## Deployment

Built as a Cloudflare Worker. No external dependencies.
...
- **Percent-encoded URIs** — if your client encodes `://` as `%3A%2F%2F`, wrap the pathname in `decodeURIComponent()` before parsing.
